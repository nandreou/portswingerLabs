...
func BruteForcer(urlPost string, usernames []byte, passwords []byte, threads *int) {
	...
	// Spliting usernames to threads
	passwordSplit := len(strings.Split(string(passwords), "\n")) / *threads

	// Split usernames
	for i := 0; i < *threads; i++ {
		for j := index; j < index+passwordSplit; j++ {
			splitedPasswords = append(splitedPasswords, strings.Split(string(passwords), "\n")[j])
		}
	...
	//Taking rest of Strings
	if restOfStrings := len(strings.Split(string(passwords), "\n")) % *threads; restOfStrings != 0 {
		for j := index; j < index+restOfStrings; j++ {
			splitedPasswords = append(splitedPasswords, strings.Split(string(usernames), "\n")[j])
		}
